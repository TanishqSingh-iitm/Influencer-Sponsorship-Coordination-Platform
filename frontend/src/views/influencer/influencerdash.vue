...
<template>
  ...
  <div style="margin-top: 30px;margin-bottom: 20px; align-self: center;" >
              <h1><span class="mdi mdi-view-dashboard"></span> Influencer Dashboard</h1>
    </div>
    <div class="dashboard">
      <div class="sponsornav">
        <h2 id="xyz">Advertisement Requests:</h2>
        <button type="submit" class="btn btn-success"   style="font-size:20px; border-radius:20px;" @click="readreviews" v-tooltip.top="'View My Reviews'">
                            
<span class="mdi mdi-comment-text-multiple"></span> Read Reviews
        </button>
      </div>
                ...
                <tbody>
                  <tr v-for="(actcamp, index) in activecampaigns" :key="activecampaigns.id">
                      ...
                      <td id="actions">
                          ...
                          <button type="submit" class="btn btn-info"  style="font-size:15px; border-radius:10px;" @click="chatsponsor(actcamp.id)" v-tooltip.top="'Message Sponsor'">                         
                            <span class="mdi mdi-cellphone-message"></span>
                          </button>
                          <button type="submit" class="btn btn-success"  style="font-size:15px; border-radius:10px;" @click="completerequest(actcamp.id)" v-tooltip.top="'Complete Request'">
                            <span class="mdi mdi-sticker-check"></span>
                          </button>
                  ...
                  <tr v-for="(inactcamp, index) in inactivecampaigns" :key="inactivecampaigns.id">
                      ...
                      <td id="actions">
                        <button type="submit" class="btn btn-primary"   style="font-size:15px; border-radius:10px;" @click="viewcampaign(inactcamp.campaignid)" v-tooltip.top="'View Campaign'">
                          ...
                          <button type="submit" class="btn btn-info"  style="font-size:15px; border-radius:10px;" @click="chatsponsor(inactcamp.id)" v-tooltip.top="'Negociate Request'">                         
                            <span class="mdi mdi-cellphone-message"></span>
                              </button>
                          <button type="submit" class="btn btn-success" v-if="inactcamp.requested_bysponsor" style="font-size:15px; border-radius:10px;" @click="approverequest(inactcamp.id)" v-tooltip.top="'Approve Request'">
                            <span class="mdi mdi-tooltip-check"></span>
                          </button>
                          <button type="submit" class="btn btn-danger" v-if="inactcamp.requested_bysponsor"  style="font-size:15px; border-radius:10px;" @click="rejectrequest(inactcamp.id)" v-tooltip.top="'Reject Request'">
                            <span class="mdi mdi-tooltip-remove"></span>
                          </button>
                          <button type="submit" class="btn btn-danger" v-if="inactcamp.requested_byinfluencer" style="font-size:15px; border-radius:10px;" @click="removerequest(inactcamp.id)" v-tooltip.top="'Remove Request'">
                            <span class="mdi mdi-tooltip-remove"></span>
                          </button>
                          <button type="submit" class="btn btn-primary"  style="font-size:15px; border-radius:10px;" @click="openModal(inactcamp.ad_id)" v-tooltip.top="'View Ad'">
                            <span class="mdi mdi-advertisements"></span>
                          </button>
                      </td>
                  </tr>
                ...
                <tbody>
                  <tr v-for="(compcamp, index) in completecampaigns" :key="completecampaigns.id">
                      ...
                      <td id="actions">
                          ...
                          <button type="submit" class="btn btn-info" data-bs-toggle="tooltip" data-bs-placement="top" data-bs-title="Tooltip on top"  style="font-size:15px; border-radius:10px;" @click="openModal(compcamp.ad_id)" v-tooltip.top="'View Ad'">
                            <span class="mdi mdi-advertisements"></span>
                              </button>
                      </td>
                  </tr>
      ...
      <div class="inactivesponsor" v-if="rejectads.length>0" >
          <div class="heading">
          ...
          <section class="table__body">
            <table>
              <thead>
                ...
                <tbody>
                  <tr v-for="(rejad, index) in rejectads" :key="rejad.id">
                      <td>{{ index + 1}}</td>
        ...
        <div class="totals" align="right">
          <span class="badge badge-pill" style="border-radius:30px;background:rgb(0, 88, 150)"> Total Rejected Requests: {{ rejectads.length }} </span>
        </div>
      </div>
...
export default {
  ...
  methods: {
          getdata() {
            axios 
            .get('http://localhost:5000/influencer/'+ this.user_id +'/adrequests',
                 {headers: {Authorization: this.auth}})
            .then((response) => {
                console.log(response);
                this.activecampaigns = response.data.dataapprove;
                this.inactivecampaigns = response.data.datareq;
                this.completecampaigns = response.data.datacomp;
                this.rejectads = response.data.datareject;
              })
            .catch((error) => {
                console.log(error);
            });
            ...
            completerequest(id){
            axios
            .put('http://localhost:5000/influencer/adrequest/'+id+'/complete',{},
                 {headers: {Authorization: this.auth}})
            .then((response) => {
                console.log(response);
                alert('Ad Request Completed Successfully');
                this.$router.go(0);
              })
            .catch((error) => {
                alert('Ad Request Completion Failed! Reason: ' + response.data.message +' Please try again');
                console.log(error);
            });
          },
          approverequest(id){
            axios
            .put('http://localhost:5000/influencer/adrequest/'+id+'/accept',{},
                 {headers: {Authorization: this.auth}})
            .then((response) => {
                console.log(response);
          ...
          rejectrequest(id){
            axios
            .put('http://localhost:5000/influencer/adrequest/'+id+'/reject',{},
                 {headers: {Authorization: this.auth}})
            .then((response) => {
                console.log(response);
          ...
          removerequest(id){
            axios
            .delete('http://localhost:5000/influencer/adrequest/'+id+'/remove',
                 {headers: {Authorization: this.auth}})
            .then((response) => {
                console.log(response);
          ...
          readreviews(){
            this.$router.push({ name: 'InfluencerReviewsPage' });
          },
          chatsponsor(id){
            this.$router.push({ name: 'InfluencerNegociationMessagePage', params: { request_id: id } });
          },
