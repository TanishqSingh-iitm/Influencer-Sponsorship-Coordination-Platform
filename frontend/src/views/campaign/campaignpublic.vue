...
<template>
  ...
  <div class="statsbody">
      ...
      <div class="navigators">
        ...
        <div class="row1">  
          <div class="square blue-gradient"> 
            ...
            <div class="square green-gradient" v-if="checked"> 
                <h4><span class="mdi mdi-bullseye"></span></h4>
                <h4>Current</h4> 
                <p>{{campaigncurrent}}</p> 
            </div> 
            <div class="square red-gradient" v-if="checked"> 
            ...
            <div class="square orange-gradient"> 
                <h4><span class="mdi mdi-calendar-clock"></span></h4>
                <h4>DaysLeft</h4> 
        ...
        <button type="button" class="btn btn-danger" style="border-radius: 20px;font-size:20px;" @click="openReportModal" v-if="campaignstatus!='Completed'" v-tooltip.bottom="'Report Campaign'"><span class="mdi mdi-comment-alert"></span> Report</button>
      </div>
            </div>
            <div class="prog">
    ...
    <div class="campaigndetails">
        <div class="detailsrow1">
          ...
          <h4 @click="sponserprofile" style="cursor: pointer;"><b>Campaign Sponsor:</b>  <u> {{ campaignsponsor }}</u> </h4>
        </div>
  ...
  <div class="addsbody">
    <div class="addsheads">
      <h2 id="dashh"><span class="mdi mdi-newspaper-variant"></span>Open Advertisements</h2>
    </div>
    <div class="activesponsor" v-if="ads.length>0">
        ...
        <section class="table__body" >
            <table>
                ...
                <tbody>
                  <tr v-for="(ad, index) in ads" :key="ad.id" v-if="checked">
                      <td>{{ index + 1}}</td>
                      <td>{{ ad.title }}</td>
					...
					<span class="mdi mdi-snapchat"  v-if="ad.preferedplatforms.snapchat"  ></span><span class="mdi mdi-linkedin"  v-if="ad.preferedplatforms.linkedin" ></span></td>
                      ...
                      <td id="actions">
                        <button type="button" class="btn btn-primary" style="border-radius: 10px; font-size: 15px;" @click="openModal(ad)" v-tooltip.bottom="'View Ad'"><span class="mdi mdi-advertisements"></span></button>
		                    <button type="button" class="btn btn-success" style="border-radius: 10px;font-size:15px;"v-tooltip.bottom="'Request Ad'" v-if="adcheck(ad.status) && userrole!='Admin'"><span class="mdi mdi-near-me"></span></button>
                      </td>
                  </tr>
                  <tr v-for="(ad, index) in filteredAds" :key="ad.id" v-if="!checked">
                      <td>{{ index + 1}}</td>
                      <td>{{ ad.title }}</td>
                      <td>&#8377 {{ ad.baseprice }}</td>
                      <td>&#8377 {{ ad.negociatedprice }}</td>
					...
					<span class="mdi mdi-snapchat"  v-if="ad.preferedplatforms.snapchat"  ></span><span class="mdi mdi-linkedin"  v-if="ad.preferedplatforms.linkedin" ></span></td>
                      ...
                      <td id="actions">
                        <button type="button" class="btn btn-primary" style="border-radius: 10px; font-size: 15px;" @click="openModal(ad)" v-tooltip.bottom="'View Ad'"><span class="mdi mdi-advertisements"></span></button>
		                    <button type="button" class="btn btn-success" style="border-radius: 10px;font-size:15px;"v-tooltip.bottom="'Apply fo Ad'" @click="applyad(ad.campaign_id,ad.id)" v-if="adcheck(ad.status)"><span class="mdi mdi-near-me"></span></button>
                      </td>
                  </tr>
                </tbody>
            </table>
    ...
    <div class="modal-content"> 
      ...
      <div class="modal fade" id="rateModal" tabindex="-1" aria-labelledby="rateModalLabel" aria-hidden="true"> 
        <div class="modal-dialog"> 
          <div class="modal-content"> 
            <div class="modal-header"> 
              <h1 class="modal-title fs-5" id="rateModalLabel">Rate Campaign</h1> 
              <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button> 
            </div> 
            <div class="modal-body"> 
              <form> 
                <div class="mb-3"> 
                  <label for="rate-influencer" class="col-form-label">Rating:</label> 
                  <select class="form-control" id="rate-influencer" v-model="rating"> 
                  ...
                  <div v-if="ratingError" class="text-danger">{{ ratingError }}</div>
                </div>
              </form> 
            </div> 
            <div class="modal-footer"> <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Close</button>
              <button type="button" class="btn btn-primary" @click="validateRatingForm">Submit Rating</button>
            </div> 
          </div> 
        </div>
      </div>
...
import Adrequestdash from '../sponsor/adrequestdash.vue';
export default {
  name: 'Campaign',
  data() {
    return {
  ...
  methods: {
          ...
          getdata(){
            axios
                .get('http://localhost:5000/campaign/'+this.campaign_id+'/publicview', {
                    headers: {
                ...
                .then(response => {
                    ...
                    this.sponsor_id = response.data.sponsor_id;
          ...
          getads(){
            axios
              ...
              getStatusStyle(status) { const styles = { Open: 'background:rgb(0, 88, 150);', Approved: 'background:rgb(76, 175, 80);', Rejected: 'background:rgb(244, 67, 54);', Requested: 'background:rgb(255, 152, 0);', InProgress: 'background:rgb(33, 150, 243);', Completed: 'background:rgb(0, 128, 128);', Pending: 'background:rgb(128, 0, 128);', Negotiated: 'background:rgb(184, 134, 11);', Closed:'background:rgb(0,0,0)' }; return styles[status] || ''; },
              openModal(ad) { 
                this.selectedAd = ad; 
    ...
    applyad(id1,id2){
      axios
            .post('http://localhost:5000/campaign/'+id1+'/applyadrequest/'+id2,{} ,{
                headers: {
                    Authorization: `${this.auth}`
                }
            })
            .then(response => {
                ...
                if(response.status == 200){
                  alert('Ad Requested Successfully');
                }
                else{
                  alert('Ad Request Failed Reason: '+response.data.message+' Please try again later');
                }
                this.$router.go(-1);
            })
            .catch(error => {
    ...
    validateReportForm() { 
      ...
      sponserprofile(){
        this.$router.push({ name: 'SponsorProfilePublicPage', params: { users_id: this.sponsor_id } });
      },
      checkuser(){
  ...
  computed: {
    filteredAds() { return this.ads.filter(ad => ad.status === 'Open'); }
  },
  watch: {
