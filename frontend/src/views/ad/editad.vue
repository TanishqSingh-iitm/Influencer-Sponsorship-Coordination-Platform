...
<template>
  ...
  <div style="margin-top:20px">
    <h2 class="display-4 " align="center">Advertisement Request Edit Portal</h2>
  </div>
  <div class="registerform">
    ...
    <div class="form-group" align="center">
        <button type="submit" id="submission" class="btn"  align="center" @click.prevent="validateform"><span class="mdi mdi-text-box-edit"></span> Edit Campaign Ad
          </button>
    </div>
</form>
  </div>
...
export default {
  ...
  methods: {
    ...
    createad(){
        axios
                .put('http://localhost:5000/ad/'+this.adid+'/edit', {
                    title: this.name,
                ...
                .then((response) => {
                    ...
                    if(response.status == 200){
                        console.log("Campaign Ad Edited Successfully");
                        this.name = '';
                        this.description = '';
                        this.target = '';
                        this.industry = '';
                        alert('Campaign Ad Edited Successfully');
                        this.$router.go(-1);
                    }
                    else{
                        console.log("Campaign Ad Edit Failed");
                        alert('Campaign Ad Edit Failed! Reason: ' + response.data.message +' Please try again');
                        this.$router.go(-1);
                    }
                })
                .catch((error) => {
